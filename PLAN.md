...
- 2026-07-01: **In-app Send Feedback — gear-menu sheet → Supabase `feedback` Edge Function (`Sources/StowerMacUI/Feedback/`, `Views/StowerFeedbackView.swift`).**
  New app-side subsystem: `StowerFeedbackClient` (header-less JSON POST to `…/functions/v1/feedback`, `--no-verify-jwt`, injectable `Transport`, 15s timeout, 2xx-wide accept, coarse `StowerFeedbackFailure` carrying no PII), `StowerFeedbackDraft` (hand-written `encode(to:)` emits only `text`/`appVersion`/`licenseID?`/`email?`; the secret `licenseKey` is structurally absent), and `StowerFeedbackFormModel` (`@MainActor @Observable`, `canSend` blocks blank/oversized/in-flight/`didSend` double-send). New config field `StowerLicenseConfig.feedbackBaseURL` (public value #4, override `STOWER_FEEDBACK_BASE_URL` in DEBUG) + `currentLicenseID()` on `StowerLicenseGating`/`StowerLicenseGate`/`StowerStartupModel` (resource id, non-nil for trial AND paid; NOT `trialBadge()?.licenseID`). The board gear menu (`licenseMenu`) is now **always enabled** with a divider-separated "Send Feedback…" item; success shows a brief bottom-edge capsule whose dwell starts on appear (not on send) and is suppressed under the undo bar. `StowerRootView` assembles the client once from `StowerLicenseConfig.resolved.feedbackBaseURL` and `Bundle.main` appVersion. Docs: new `Docs/Feedback.md`; `Lifecycle.md` §3 (two Edge Function routes), `EnvironmentVariables.md` §3 (config row 4), `licensing-contract.md` (gear always-enabled) synced. New tests: `StowerFeedbackClientTests`, `StowerFeedbackFormModelTests`, `StowerFakeLicenseGate` `currentLicenseID`. Shipped on branch `send-feedback` (PR #40).
